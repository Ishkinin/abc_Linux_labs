...
int main(int argc, char *argv[])
{
    ...
    key = new_key();
    if (key == -1) {
        printf("ftok() не удалось по какой-то причине\n\n");
        return 1;
    }
...
{
    sem_wait(&m->sem);
    printf("Основной Проц: ожиданиет 5 сообщений\n\n");
    sem_post(&m->sem);

    for (int i = 0; i < 5; i++) {
        sem_wait(&m->sem_new_msg); // Ожидание новой сообщеньки
        sem_wait(&m->sem);
        printf("Основной Проц: получение сообщение от процесса клиента с PID %i:\n%s\n\n",
    ...
    sem_wait(&m->sem);
    printf("Основные Проц: получено пять Сообщений!\n\n");
    sem_post(&m->sem);
